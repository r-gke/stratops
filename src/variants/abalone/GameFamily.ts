...
import {charToPiece, FenError, InvalidFen, parseFen as parseFenVariant, parseFullMoves, parseHalfMoves, parsePlayerTurn, parsePliesRemainingThisTurn, parseScore,} from '../../fen';
import * as fp from '../../fp';
import {defaultSetup, type Setup} from '../../setup';
import type {Outcome, Piece, PlayerIndex} from '../../types';
...
export abstract class GameFamily extends Variant {
	...
	static override computeMoveNotation(move: ExtendedMoveInfo): string {
		return this.computeMoveNotationCore(MoveNotation.PlayStrategy, move);
	}
	
	protected static computeMoveNotationCore(notation: MoveNotation, move: ExtendedMoveInfo): string {
		...
		if (board.isOk) {
			...
			if (cFrom !== undefined) {
				...
				if (n > 0) return this.computeMoveNotationCoreCore(notation, board.value, from, to, vect, n, cFrom);
			}
		}
	...
	protected static computeMoveNotationCoreCore(notation: MoveNotation, board: Board, from: Pos, to: Pos, vect: Pos, n: number, cFrom: Piece): string {
		const uvect = div(n, vect),
			neighVectors = getNeighVectors();
		
		return includes(neighVectors, uvect)
			? this.computeMoveNotationCore_line(notation, board, neighVectors, from, to, vect, n, uvect, cFrom) // In-line move
			: this.computeMoveNotationCore_jump(notation, board, neighVectors, from, to, vect, n, uvect, cFrom); // Broadside move
	}
